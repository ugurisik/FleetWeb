...
class userAuthMw
{
    ...
    public function login($username, $password)
    {
        ...
        if (!empty($data)) {
            if (password_verify($password, $data['Password'])) {
                if ($data['Role'] == 2 || $data['Role'] == 1 || $data['Role'] == 0) {
                    if ($data['Status']) {
    ...
    public function isLogged($isRedirectMain = true)
    {
        ...
        if (empty($data)) {
            ...
            if (!$isRedirectMain) {
                $this->functions->Redirect(SITE_URL  . '/login');
            }
        } else {
            if ($LoginToken == $data['LoginToken']) {
                if (time() - $LoginTokenTime > $LoginTokenTimeLimit) {
                    ...
                    if ($Login) {
                        if ($isRedirectMain) {
                            $this->functions->Redirect(SITE_URL . '/forum');
                        }
                    } else {
                        $this->session->allSessionDelete();
                        if (!$isRedirectMain) {
    ...
    {
        $data = $this->db->getData('swp_user_information');
        $datas = [];
        foreach ($data as $user) {
            $datas[$user['UserGuid']] = $user['FirstName'] . ' ' . $user['LastName'];
        }
        return $datas;
    }
}
